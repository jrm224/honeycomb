...
def calculate_action(bee_state, hive, parameters):
    ...
    if state == 0:   # Searching for Pulp
        ...
        else:  # Chance of staying depends on num of tall walls
            if rand.random() <= D[len(tall_walls[0])]:
...
hive = Hive()
print(hive.get_cells()[(0,0)].walls)
